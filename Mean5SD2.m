...
anatomy_label = {'Heart', 'Myocardium', 'excludeContour', 'MyoReference', 'noReflowAreaContour'};
...
for i = 1:length(Names)
    ...
    if length(ls(cat(2, base_dir, name, '/', label, '/', anatomy))) > 2
        [mask, dicom_idx] = ReadMatFile3D(name, label, anatomy, base_dir);
    end
    ...
    if length(ls(cat(2, base_dir, name, '/', label, '/', anatomy_label{5}))) > 2
        load(cat(2, base_dir, name, '/', label, '/', anatomy_label{5}, '/', label, '_NoReFlow.mat'));
    end
    if isempty(mask) || isempty(myoRefCell)
        ...
        if ~isempty(NoReFlowCell)
            NoReFlowMat = NoReFlowCell{1};
            noreflow_idx = NoReFlowCell{2};
        else
            noreflow_idx = [];
        end
        ...
        for j = 1:length(dicom_idx)
            ...
            noref_ind = find(dicom_idx(j) == noreflow_idx);
            if ~isempty(ref_ind)
            ...
            if ~isempty(noref_ind)
                compositeIm(:,:,j) = compositeIm(:,:,j) + NoReFlowMat(:,:,noref_ind) * 2;
            end
        end
%                % Display Images
%                            figure();
...
%                            end
        ...
        % Apply NoReflowArea
        norefIm = compositeIm == 3;
        infarct_ex_masked = norefIm + infarct_ex_masked;
